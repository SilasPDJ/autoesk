...
class DownloadGinfessGui(WDShorcuts, SetPaths, ExcelToData):
    ...
    def ginfess_table_valores_html_code(self):
        ...
        for i in range(10):
            ...
            download_notas = True
            if download_notas:
                notasdownload = driver.find_elements_by_class_name('viewNota')

                for e, nf in enumerate(notasdownload):
                    driver.switch_to_window(driver.window_handles[0])

                    self.click_ac_elementors(nf)
                self.download_pdf_final()
            driver.switch_to_window(driver.window_handles[0])
            # ----------------------------------------------------------------------- download
    ...
    def download_pdf_final(self, ext='pdf'):
        import os

        ext.replace('.', '')

        nfs = 'NotasFiscaisPDF'
        download_path = self.client_path + f'\\{nfs}'
        try:
        ...
        driver = self.driver
        for win in self.driver.window_handles[1:]:
            driver.switch_to_window(win)
            nf_number = driver.current_url.split('&')
            nf_number = nf_number[-2]
            nf_number = ''.join(nf_number.split('=')[-1])

            self.driver.find_element_by_link_text('Exportar PDF').click()
            src_pdf = self.newest_file(self.client_path, ext)
            dst_pdf = download_path + f'\\{nf_number}.{ext}'
            if os.path.isfile(src_pdf):
                self.move_file(src_pdf, dst_pdf)
                driver.close()

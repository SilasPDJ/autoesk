...
class MainApp(QMainWindow, DisplaySheets, TuplasTabelas):
    def __init__(self, parent=None):
        ...
        for e, el_grid in enumerate(self.el_grid_setting(0, len(bts_plans), mt=1, start_col=1)):
            ...
            # Conecta o botão do Download_Gifess (self.when_ginfess_bt)
            # if sh_ui_name == 'G5_ISS':
            if 'ISS' in sh_ui_name:
                new.clicked.connect(lambda: self.whenGinfessBt.setDisabled(False))
            else:
    ...
    def whenginfessbt_call_funcs(self):
        ...
        self.load_tables(2)
        DownloadGinfessGui(JsonDateWithImprove.load_json(self.now_selection_json))

...
class DisplaySheets(QWidget, SetPaths, ExcelToData):
    ...
    # Create tables
    def create_tables(self, table, dataframe, df_id):
        # Row count
        ...
        table.itemSelectionChanged.connect(lambda: self.index_for_data(table, df, df_id))

    def index_for_data(self, table, df, df_id):
        rows = (idx.row() for idx in table.selectionModel().selectedRows())
        dc_df = df.to_dict
        sh_names = list(self.parse_sh_name(False))[df_id]
        print(sh_names)
        for r in rows:
            print(r)
            # print(df.title)
